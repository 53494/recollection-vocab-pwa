...
## 项目简介
**Recollection** 是面向英语专业学生的轻量化深度学习型词汇记忆PWA网页应用，核心学习逻辑为「中文释义→自主英文造句 + AI语法诊断」，帮助使用者掌握单词地道搭配、语境与实用句式，完整落地教育类AI产品雏形Demo。
...
受控服务端 AI 批改（Cloudflare Worker 代理） | Web Speech 语音朗读
vite-plugin-pwa + Workbox（离线PWA能力）

## 核心产品功能
1. 单词词书系统：分级词汇库、本地离线存储单词数据，自定义词书导入
2. 核心学习流程：中文提示自主造句，通过受控服务端代理调用大模型完成英文句子语法诊断、用词优化、搭配解析
...
- AI 批改默认调用已部署的 Cloudflare Worker；如需切换代理地址，可在构建时设置公开变量 `VITE_AI_PROXY_URL`。上游 API Key 只保存在 Cloudflare Secret 中，不应写入前端环境文件。
